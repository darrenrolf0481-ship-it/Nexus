...
# Add parent directory to sys.path to find soul_loader
...
@dataclass
class AgentConfig:
    BRIDGE_TARGET: str = "http://localhost:8000"
...
@app.get("/api/v1/bridge/sync")
async def bridge_sync():
    try:
        r = requests.get("http://localhost:8000/api/v1/neural/status", timeout=2)
        return {"bridge_status": "linked", "star_city_data": r.json()}
    except Exception as e:
        return {"bridge_status": "silent", "error": str(e)}
